...
var (
	walletDataDirectory = monautil.AppDataDir("monawallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func sweep() error {
	...
	var totalSwept monautil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := monautil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount monautil.Amount) bool {
	return amount >= 0 && amount <= monautil.MaxSatoshi
}
