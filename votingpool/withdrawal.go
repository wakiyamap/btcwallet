...
func (s *WithdrawalStatus) Fees() monautil.Amount {
	return s.fees
}
...
func (o OutBailmentOutpoint) Amount() monautil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() monautil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() monautil.Amount {
		return monautil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total monautil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total monautil.Amount) {
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold monautil.Amount) (
...
func (w *withdrawal) finalizeCurrentTx() error {
	...
	for _, txOut := range tx.outputs {
		...
		amtFulfilled := monautil.Amount(0)
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := monautil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := monautil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold monautil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold monautil.Amount) (*WithdrawalStatus, error) {
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *monautil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*monautil.AddressScriptHash))
	if err != nil {
