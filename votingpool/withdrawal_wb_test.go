...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	output1Amount := monautil.Amount(2)
	output2Amount := monautil.Amount(3)
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	requestAmount := monautil.Amount(5)
	bigInput := int64(3)
	smallInput := int64(2)
	...
	if tx1.outputs[0].amount != monautil.Amount(bigInput) {
	...
	if tx2.outputs[0].amount != monautil.Amount(smallInput) {
...
func TestFulfillRequestsNoSatisfiableOutputs(t *testing.T) {
	...
	request := TstNewOutputRequest(
		t, 1, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", monautil.Amount(3e6), pool.Manager().ChainParams())
	changeStart := TstNewChangeAddress(t, pool, seriesID, 0)
...
func TestFulfillRequestsNotEnoughCreditsForAllRequests(t *testing.T) {
	...
	out1 := TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", monautil.Amount(3e6), net)
	out2 := TstNewOutputRequest(
		t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhTQ86XG", monautil.Amount(2e6), net)
	out3 := TstNewOutputRequest(
		t, 3, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", monautil.Amount(5e6), net)
...
func TestRollbackLastOutputMultipleInputsRolledBack(t *testing.T) {
	...
	for i, amount := range []monautil.Amount{4, 3, 2} {
...
func TestRollBackLastOutputInsufficientOutputs(t *testing.T) {
	...
	output := &WithdrawalOutput{request: TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", monautil.Amount(3), &chaincfg.MainNetParams)}
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	checkTxChangeAmount(t, firstTx, monautil.Amount(4))
	...
	checkTxChangeAmount(t, secondTx, monautil.Amount(3))
}
...
func TestWithdrawalTxAddChange(t *testing.T) {
	...
	tx.calculateFee = TstConstantFee(monautil.Amount(fee))
...
func TestWithdrawalTxInputTotal(t *testing.T) {
	...
	if tx.inputTotal() != monautil.Amount(5) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), monautil.Amount(5))
	}
}
...
func TestWithdrawalTxOutputTotal(t *testing.T) {
	...
	if tx.outputTotal() != monautil.Amount(4) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), monautil.Amount(4))
	}
}
...
func TestSignMultiSigUTXOPkScriptNotP2SH(t *testing.T) {
	...
	mgr := pool.Manager()
	tx := createWithdrawalTx(t, dbtx, pool, []int64{4e6}, []int64{})
	addr, _ := monautil.DecodeAddress("1MirQ9bwyQcGVJPwKUgapu5ouK2E2Ey4gX", mgr.ChainParams())
	pubKeyHashPkScript, _ := txscript.PayToAddrScript(addr.(*monautil.AddressPubKeyHash))
	msgtx := tx.toMsgTx()
...
func TestSignMultiSigUTXORedeemScriptNotFound(t *testing.T) {
	...
	// script.
	addr, _ := monautil.DecodeAddress("3Hb4xcebcKg4DiETJfwjh8sF4uDw9rqtVC", mgr.ChainParams())
	if _, err := mgr.Address(addrmgrNs, addr); err == nil {
		t.Fatalf("Address %s found in manager when it shouldn't", addr)
	}
	msgtx := tx.toMsgTx()

	pkScript, _ := txscript.PayToAddrScript(addr.(*monautil.AddressScriptHash))
...
func TestStoreTransactionsWithChangeOutput(t *testing.T) {
	...
	inputTotal := monautil.Amount(0)
	for _, debit := range txDetails.Debits {
		inputTotal += debit.Amount
	}
	if inputTotal != monautil.Amount(5e6) {
		t.Fatalf("Unexpected input amount; got %v, want %v", inputTotal, monautil.Amount(5e6))
	}
...
func createWithdrawalTxWithStoreCredits(t *testing.T, dbtx walletdb.ReadWriteTx, store *wtxmgr.Store, pool *Pool,
	...
	for i, amount := range outputAmounts {
		request := TstNewOutputRequest(
			t, uint32(i), "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", monautil.Amount(amount), net)
...
func checkMsgTxOutputs(t *testing.T, msgtx *wire.MsgTx, requests []OutputRequest) {
	...
	for i, request := range requests {
		...
		gotAmount := monautil.Amount(txOut.Value)
...
func checkTxChangeAmount(t *testing.T, tx *withdrawalTx, amount monautil.Amount) {
	if !tx.hasChange() {
...
func checkLastOutputWasSplit(t *testing.T, w *withdrawal, tx *withdrawalTx,
	origAmount, newAmount monautil.Amount) {
