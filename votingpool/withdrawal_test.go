...
import (
	...
	vp "github.com/wakiyamap/monawallet/votingpool"
)
...
func TestStartWithdrawal(t *testing.T) {
	...
	checkWithdrawalOutputs(t, status, map[string]monautil.Amount{address1: 4e6, address2: 1e6})

	if status.Fees() != monautil.Amount(1e3) {
		t.Fatalf("Wrong amount for fees; got %v, want %v", status.Fees(), monautil.Amount(1e3))
	}
...
func checkWithdrawalOutputs(
	t *testing.T, wStatus *vp.WithdrawalStatus, amounts map[string]monautil.Amount) {
