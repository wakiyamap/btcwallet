...
func TstCreateSeriesCredits(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, seriesID uint32, amounts []int64) []credit {
	...
	for i := range msgTx.TxOut {
		c := wtxmgr.Credit{
			...
			Amount:   monautil.Amount(msgTx.TxOut[i].Value),
...
func TstNewOutputRequest(t *testing.T, transaction uint32, address string, amount monautil.Amount,
	net *chaincfg.Params) OutputRequest {
	addr, err := monautil.DecodeAddress(address, net)
	if err != nil {
...
func TstConstantFee(fee monautil.Amount) func() monautil.Amount {
	return func() monautil.Amount { return fee }
}
...
func createAndFulfillWithdrawalRequests(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, roundID uint32) withdrawalInfo {
	...
	dustThreshold := monautil.Amount(1e4)
