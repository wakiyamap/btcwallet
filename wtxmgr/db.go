...
func fetchMinedBalance(ns walletdb.ReadBucket) (monautil.Amount, error) {
	...
	return monautil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt monautil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (monautil.Amount, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (monautil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return monautil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (monautil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return monautil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (monautil.Amount, error) {
	...
	return monautil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (monautil.Amount, error) {
	...
	return monautil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = monautil.Amount(byteOrder.Uint64(it.cv))
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount monautil.Amount, block *Block, credKey []byte) error {
...
func valueUnminedCredit(amount monautil.Amount, change bool) []byte {
	v := make([]byte, 9)
	byteOrder.PutUint64(v, uint64(amount))
	if change {
		v[8] = 1 << 1
...
func fetchRawUnminedCreditAmount(v []byte) (monautil.Amount, error) {
	if len(v) < 9 {
...
func fetchRawUnminedCreditAmountChange(v []byte) (monautil.Amount, bool, error) {
	if len(v) < 9 {
		str := "short unmined credit value"
		return 0, false, storeError(ErrData, str, nil)
	}
	amt := monautil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
