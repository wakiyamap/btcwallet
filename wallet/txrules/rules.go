...
func GetDustThreshold(scriptSize int, relayFeePerKb monautil.Amount) monautil.Amount {
	...
	relayFee := monautil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount monautil.Amount, scriptSize int, relayFeePerKb monautil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb monautil.Amount) bool {
	...
	return IsDustAmount(monautil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb monautil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > monautil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb monautil.Amount, txSerializeSize int) monautil.Amount {
	fee := relayFeePerKb * monautil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > monautil.MaxSatoshi {
		fee = monautil.MaxSatoshi
	}
