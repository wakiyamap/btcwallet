...
func (w *Wallet) MakeMultiSigScript(addrs []monautil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*monautil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*monautil.AddressScriptHash, error) {
	var p2shAddr *monautil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = monautil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*monautil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
