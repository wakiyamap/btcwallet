...
// InputSourceError.
type InputSource func(target monautil.Amount) (total monautil.Amount, inputs []*wire.TxIn,
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []monautil.Amount
	TotalInput      monautil.Amount
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []monautil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := monautil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
