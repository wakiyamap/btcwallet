...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentInputValues := make([]monautil.Amount, 0, len(unspents))
	f := func(target monautil.Amount) (monautil.Amount, []*wire.TxIn, []monautil.Amount, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil, nil)
			currentTotal += monautil.Amount(u.Value)
			currentInputs = append(currentInputs, nextInput)
			currentInputValues = append(currentInputValues, monautil.Amount(u.Value))
		}
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         monautil.Amount
		ChangeAmount     monautil.Amount
