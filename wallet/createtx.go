...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := monautil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]monautil.Amount, 0, len(eligible))

	return func(target monautil.Amount) (monautil.Amount, []*wire.TxIn,
		[]monautil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr monautil.Address) (*btcec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr monautil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb monautil.Amount) (tx *txauthor.AuthoredTx, err error) {
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := monautil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []monautil.Amount) error {
