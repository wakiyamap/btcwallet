...
func (a *managedAddress) ExportPrivKey() (*monautil.WIF, error) {
	...
	return monautil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := monautil.NewAddressWitnessPubKeyHash(
		...
		address, err = monautil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = monautil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := monautil.NewAddressScriptHashFromHash(
