...
type config struct {
	...
	TestNet4      bool                    `long:"testnet" description:"Use the test Bitcoin network (version 3) (default mainnet)"`
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of monad RPC server to connect to (default localhost:9400, testnet: localhost:19400, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with monad"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	BtcdUsername     string                  `long:"monadusername" description:"Username for monad authentication"`
	BtcdPassword     string                  `long:"monadpassword" default-mask:"-" description:"Password for monad authentication"`
	...
	LegacyRPCListeners     []string                `long:"rpclisten" description:"Listen for legacy RPC connections on this interface/port (default port: 9402, testnet: 19402, simnet: 18554)"`
	LegacyRPCMaxClients    int64                   `long:"rpcmaxclients" description:"Max number of legacy RPC clients for standard connections"`
	LegacyRPCMaxWebsockets int64                   `long:"rpcmaxwebsockets" description:"Max number of legacy RPC websocket connections"`
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and monad authentication (if monadusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and monad authentication (if monadpassword is unset)"`
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.UseSPV {
		...
		if cfg.DisableClientTLS {
			...
			// If CAFile is unset, choose either the copy or local monad cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local monad and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						monadCertExists, err := cfgutil.FileExists(
							monadDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						...
						if monadCertExists {
							cfg.CAFile.Value = monadDefaultCAFile
						}
					}
				}
			}
	...
	// If the monad username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for monad and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for monad and wallet.
	if cfg.BtcdUsername == "" {
