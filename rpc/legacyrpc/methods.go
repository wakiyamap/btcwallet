...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by monawallet due to
...
// unsupported by monawallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &btcjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by monawallet",
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*monautil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := monautil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]monautil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := monautil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
// information about the current state of monawallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to monad for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (monautil.Address, error) {
	addr, err := monautil.DecodeAddress(s, params)
	if err != nil {
...
// appear in the blockchain, or any tx that has arrived in the monad mempool).
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := monautil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// monad returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to monad.  Therefore, create a
	...
	// texts in a non-internal monad package.
...
// and amounts.
func makeOutputs(pairs map[string]monautil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
func sendPairs(w *wallet.Wallet, amounts map[string]monautil.Amount,
	account uint32, minconf int32, feeSatPerKb monautil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	pairs := map[string]monautil.Amount{
		cmd.ToAddress: amt,
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// TODO: really we probably should look these up with monad anyway to
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := monautil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	// querying monad with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*monautil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*monautil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := monautil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := monautil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *monautil.AddressPubKeyHash: // ok
		return bytes.Equal(monautil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *monautil.AddressPubKey: // ok
