...
(account 0) from a testnet3 wallet listening on `localhost:19402` in several
different languages:
...
func main() {
	...
	conn, err := grpc.Dial("localhost:19402", grpc.WithTransportCredentials(creds))
	if err != nil {
		fmt.Println(err)
		return
	}
...
```bash
$ protoc -I/path/to/monawallet/rpc --cpp_out=. --grpc_out=. \
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by monawallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.monawallet/rpc.cert"s;
    }();
    ...
    auto channel = grpc::CreateChannel("localhost:19402", creds);
...
```PowerShell
PS> & protoc.exe -I \Path\To\monawallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by monawallet).
            ...
            var channel = new Channel("localhost:19402", new SslCredentials(cert));
            try
            {
...
var certPath = path.join(process.env.HOME, '.monawallet', 'rpc.cert');
...
var client = new walletrpc.WalletService('localhost:19402', creds);
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.monawallet', 'rpc.cert')
    ...
    channel = implementations.secure_channel('localhost', 19402, creds)
